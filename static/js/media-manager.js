...
async function reloadMediaLibrary() {
  ...
  if (!grid) return;
  grid.innerHTML = '<div class="col-12 text-center text-secondary py-5">Se încarcă fișierele...</div>';
  ...
  try {
    ...
    if (data.ok && Array.isArray(data.files)) {
      ...
      grid.innerHTML = '<div class="col-12 text-center text-danger py-5">Eroare la încărcarea fișierelor.</div>';
    }
  } catch (err) {
    console.error(err);
    grid.innerHTML = '<div class="col-12 text-center text-danger py-5">Eroare de rețea la încărcare.</div>';
  }
}
...
function renderMediaGrid(files) {
  ...
  if (files.length === 0) {
    grid.innerHTML = '<div class="col-12 text-center text-secondary py-5">Nicio imagine găsită în bibliotecă.</div>';
    return;
  }
...
async function uploadSelectedMediaFiles(input) {
  ...
  try {
    ...
    if (data.ok) {
      input.value = '';
      const libTabBtn = document.getElementById('tab-media-library');
      if (libTabBtn) {
        const tab = new bootstrap.Tab(libTabBtn);
        tab.show();
      }
      reloadMediaLibrary();
    } else {
...
function selectHeroImageFromMedia(file) {
  ...
  if (previewDiv) {
    previewDiv.innerHTML = '<img src="' + file.file_url + '" alt="Hero image" style="max-width: 200px; height: auto; border-radius: 6px;" />';
  }
}
